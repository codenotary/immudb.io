...
In the following example, we introduce an optional parameter named `documentIdFieldName`. This
parameter denotes the field name that serves as a document's unique identifier. When this parameter
is omitted, the system defaults to using an automatically generated field, `_id`. For both `_id`
and userID, the document's identifier is a hexadecimal binary value.

>  NOTE: Remember that hexadecimal string representation of binary value implies that the ID must
...
### Insert document with provisioned ID

If you don't want to get autogenerated document ID use your custom identifier. For more details [see](#create-collection)
...
-d '{
  "documents": [
    {"userID": "1234", "name":"John", "surname":"Doe"}
  ]
}'
```
