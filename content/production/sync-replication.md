...
weight: 140
aliases: ["/master/production/sync-replication/"]
---
...
Make sure you already have [immudb installed](/running/download/).
...
3. In a new terminal, use the [`immuadmin`](/connecting/clitools/) command to create a database on the primary server:
...
4. Use the [`immuadmin`](/connecting/clitools/) command to create a database on the replica server:
...
1. Use the [`immuclient`](/connecting/clitools/) command to commit a transaction on the primary server:
...
2. Verify the transaction on the replica server using the [`immuclient`](/connecting/clitools/) command:
