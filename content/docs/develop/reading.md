...
title: "Reading"
weight: 300
aliases: ["/master/develop/reading/", "/develop/reading/"]
---
{{% callout "tip" %}}
...
It's possible also to use dedicated [auditors](/docs/production/auditor/) to ensure the database consistency, but the pattern in which every client is also an auditor is the more interesting one.

## Get and Set

`Get`/`VerifiedGet` and `Set`/`VerifiedSet` methods allow for basic operations on a Key Value level. In addition, `GetAll` and `SetAll` methods allow for adding and reading in a single transaction. See [transactions chapter](/docs/develop/transactions/) for more details.
...
{{% tab "Others" %}}
If you're using another development language, please refer to the [immugw](/docs/connecting/immugw/) option.
{{% /tab %}}
...
{{% tab "Go" %}}
{{< snippet "/code-examples/go/develop-kv-get-at-since/main.go" >}}
{{% /tab %}}
...
{{% tab "Go" %}}
{{< snippet "/code-examples/go/develop-kv-get-at-revision/main.go" >}}
{{% /tab %}}
...
{{% tab "Go" %}}
{{< snippet "/code-examples/go/develop-kv-get-at-txid/main.go" >}}
{{% /tab %}}
...
{{< snippet "/code-examples/go/develop-kv-preconditions/main.go" >}}
{{% /tab %}}
