...
weight: 600
aliases: ["/master/releasenotes/", "/releasenotes/"]
---
...
- **PostgreSQL compatibility**: deep wire-protocol and SQL coverage validated end-to-end against Rails / ActiveRecord, Gitea, Django, GORM, XORM, SQLAlchemy, golang-migrate, JDBC, and pgAdmin. Real `pg_catalog` and `information_schema` system tables replace ~1500 LOC of legacy regex handlers. See the [PostgreSQL Compatibility](/docs/develop/sql/pg/) page.
- **[SQL Diff Query](/docs/develop/sql/sqldiff/)**: new `SELECT _diff_action, ... FROM (DIFF OF table) SINCE TX n UNTIL TX m` syntax for transaction-range diffs against the immutable log.
- **[Audit Logging](/docs/develop/sql/auditlog/)**: structured, immutable, async audit trail for every gRPC operation, queryable via the standard Scan API and verifiable like any other immudb data.
...
- `FULL OUTER JOIN`, `LATERAL`, `CROSS JOIN`, `NATURAL JOIN`, `JOIN ... USING` — see [Joins](/docs/develop/sql/joins/)
- `EXISTS`, `IN`, correlated and scalar subqueries
- `WITH` and `WITH RECURSIVE` CTEs — see [CTEs](/docs/develop/sql/ctes/)
- `EXCEPT`, `INTERSECT` set operations
- Window functions `ROW_NUMBER`, `RANK`, `DENSE_RANK`, `LAG`, `LEAD`, `FIRST_VALUE`, `LAST_VALUE`, `NTILE`, plus window aggregates — see [Window Functions](/docs/develop/sql/windowfunctions/)
- `CREATE`/`DROP VIEW`, `CREATE`/`DROP SEQUENCE`, `ALTER COLUMN`, `FOREIGN KEY` (parsed), `DEFAULT` values, `SAVEPOINT` / `ROLLBACK TO` / `RELEASE` — see [Views & Sequences](/docs/develop/sql/views/)
