...
- **Verify your data without sacrificing performance.** Data stored in immudb is cryptographically coherent and verifiable, and proving it costs little enough that immudb still handles millions of transactions per second. It runs as a lightweight service, or embedded in your application as a library.
...
immudb can be run as full database server with [replicas](/docs/production/replication/) or easily [embedded](/docs/embedded/embedding/) as a lightweight database into application.
...
Check [auditor](/docs/production/auditor/) section for additional details.
