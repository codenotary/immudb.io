...
title: "Reading"
weight: 300
aliases: ["/master/develop/reading/"]
---
{{% callout "tip" %}}
...
It's possible also to use dedicated [auditors](/production/auditor/) to ensure the database consistency, but the pattern in which every client is also an auditor is the more interesting one.

## Get and Set

`Get`/`VerifiedGet` and `Set`/`VerifiedSet` methods allow for basic operations on a Key Value level. In addition, `GetAll` and `SetAll` methods allow for adding and reading in a single transaction. See [transactions chapter](/develop/transactions/) for more details.
...
{{% tab "Others" %}}
If you're using another development language, please refer to the [immugw](/connecting/immugw/) option.
{{% /tab %}}
